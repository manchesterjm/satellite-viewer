...
# the latest pull.
#
# Scheduled to run every 3 days. CelesTrak rate-limits the full-catalog
...
try {
    ...
    # Skip if the most recent file is < 2.5 days old (safety against rapid re-runs)
    $latest = Get-ChildItem $tleDir -Filter "full_catalog_*.tle" -ErrorAction SilentlyContinue |
              Sort-Object LastWriteTime -Descending | Select-Object -First 1
    if ($latest -and ((Get-Date) - $latest.LastWriteTime).TotalDays -lt 2.5) {
    ...
    # Keep only the 8 most recent datestamped files (~24 days of history)
    $keep = 8
